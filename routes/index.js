...
//       { lastMessage: chat._id, updatedAt: Date.now() },
//       { upsert: true, new: true }
//     );

//     res.status(201).json({ chat, conversation });
//   } catch (error) {
...
// router.get('/chat/:userId/:receiverId', async (req, res) => {
//   const { userId, receiverId } = req.params;

//   try {
//     const chats = await chat.find({ senderId: userId, receiverId }).populate('senderId receiverId');
//     res.json(chats);
//   } catch (error) {
...
//   try {
//     const story = new Story({ userId, mediaUrl, caption });
//     await story.save();
...
//   try {
//     const story = await Story.findByIdAndUpdate(req.params.id, { $addToSet: { views: userId } }, { new: true });
//     res.json(story);
//   } catch (error) {
...
router.get('/chat/:userId/:receiverId', async (req, res) => {
  ...
  try {
    const chats = await chatModel.find({ senderId: userId, receiverId }).populate('senderId receiverId');
    res.json(chats);
  } catch (error) {
